...
def save_visualizations(df: pd.DataFrame, monthly: pd.DataFrame, perf: pd.DataFrame, preds: pd.DataFrame, fi: pd.DataFrame, future_fc: pd.DataFrame):
    # 1 category sales
...
def save_tables(df: pd.DataFrame, monthly: pd.DataFrame, performance: pd.DataFrame, preds: pd.DataFrame, future_fc: pd.DataFrame):
    category_table = df.groupby('Category').agg(Total_Sales=('Sales', 'sum'), Total_Profit=('Profit', 'sum')).round(2).sort_values('Total_Sales', ascending=False)
    region_table = df.groupby('Region').agg(Total_Sales=('Sales', 'sum'), Total_Profit=('Profit', 'sum')).round(2).sort_values('Total_Sales', ascending=False)
    top_products = df.groupby('Product Name').agg(Total_Sales=('Sales', 'sum'), Total_Profit=('Profit', 'sum')).round(2).sort_values('Total_Sales', ascending=False).head(10)
