...
st.set_page_config(
    page_title='Retail Sales Intelligence & Forecasting System',
    page_icon='📊',
    layout='wide'
)
...
def load_data():
    try:
        logging.info("Loading dashboard data")

        df = pd.read_csv(
            DATA_DIR / 'cleaned_superstore_final.csv',
            parse_dates=['Order Date', 'Ship Date']
        )
        monthly = pd.read_csv(
            OUT_DIR / 'monthly_sales_series.csv',
            parse_dates=['Date']
        )
        forecast = pd.read_csv(
            OUT_DIR / 'future_6_month_forecast.csv',
            parse_dates=['Date']
        )
        ...
        logging.info("Dashboard data loaded successfully")
        return df, monthly, forecast, performance, fi, summary

    except FileNotFoundError as e:
        logging.error("Required file not found: %s", str(e), exc_info=True)
        st.error("A required file is missing. Please run `python src/build_project_assets.py` first.")
        st.stop()

    except Exception as e:
        logging.error("Unexpected error in load_data: %s", str(e), exc_info=True)
        st.error("An unexpected error occurred while loading dashboard data.")
        st.stop()
...
try:
    ...
    logging.info("Rendering Streamlit dashboard")
    ...
    if filtered.empty:
        logging.warning("No data available after applying filters")
        st.warning("No data matches the selected filters. Please choose different filter values.")
        st.stop()
    ...
    with dashboard_tab:
        ...
        monthly_filtered = (
            filtered.groupby(pd.Grouper(key='Order Date', freq='MS'))['Sales']
            .sum()
            .reset_index()
        )
        fig_monthly = px.line(
        ...
        scatter_df = filtered.sample(min(3000, len(filtered)), random_state=42)
    ...
    with forecast_tab:
        ...
        try:
            test_pred = pd.read_csv(
                OUT_DIR / 'forecast_test_predictions.csv',
                parse_dates=['Date']
            )
        except FileNotFoundError as e:
            logging.error("Forecast test predictions file missing: %s", str(e), exc_info=True)
            st.error("Forecast prediction file is missing. Please rerun `python src/build_project_assets.py`.")
            st.stop()
        except Exception as e:
            logging.error("Error loading forecast test predictions: %s", str(e), exc_info=True)
            st.error("An error occurred while loading forecast prediction data.")
            st.stop()

        fig_eval = px.line(
            test_pred.melt(id_vars='Date', value_vars=['Actual Sales', 'Predicted Sales']),
            x='Date',
            y='value',
            color='variable',
            markers=True,
        ...
        fi_chart = px.bar(
            fi.head(10).sort_values('Importance'),
            x='Importance',
            y='Feature',
            orientation='h',
            title='Top Forecast Features'
        )
    ...
    logging.error("Unhandled error in app.py: %s", str(e), exc_info=True)
    st.error("An unexpected application error occurred. Please check `logs/app.log` for details.")